  ...
  if (msgHdr->msgType == MessageType::CLIENT_REQUEST) {
    ...
    if (request.ParseFromArray(msgBuffer + sizeof(MessageHeader),
      ...
      if (proxyAddressMap_.get(request.proxyid()) == 0) {
         ...
         * multiple shard. The machine-id concats shard-id becomes a unqiue
      ...
      RequestBody* rb =
          new RequestBody(request.sendtime() + request.bound(),
                          CONCAT_UINT32(request.clientid(), request.reqid()),
                          request.key(), request.proxyid(), request.command());
...
void Replica::ProcessTd(int id) {
  ...
  while (status_ != ReplicaStatus::TERMINATED) {
    ...
    if (processQu_.try_dequeue(rb)) {
      VLOG(1) << "Processing..." << ((rb->reqKey) >> 32) << "\t"
              << (uint32_t)(rb->reqKey) << "\t"
      ...
      if (amLeader) {
        ...
        if (duplicateLogIdx > 0) {
          ...
          if (duplicateLogIdx == 0) {
            ...
            // unsynced, follower does not handle it, for fear of data race with
            // other threads (e.g. GarbageCollectTd) But this will not harm
            // liveness, because eventually the request will become synced
            // request, so the former if-branch will handle it and resend a slow
            // reply
            delete rb;
          }
        }
...
void Replica::ProcessRequest(const RequestBody* rb, const bool isSyncedReq,
  ...
  if (isSyncedReq) {
    ...
    if (prev != NULL) {
      prev->nextLogId =
          logId;  // This is very important, to establish the connection based
    ...
    if (minUnSyncedLogIdByKey_[rb->opKey] < CONCURRENT_MAP_START_INDEX) {
      ...
      // After switching it from the dummy value (CONCURRENT_MAP_START_INDEX-1)
      // to a valid logId, the following update will all be charged by
      // IndexRecvTd
...
void Replica::FastReplyTd(int id, int cvId) {
  ...
  while (status_ != ReplicaStatus::TERMINATED) {
    ...
    // Before encoding crashVector into hash, check whether the crashVector (cv)
    // is the freshest one
    ...
    if (fastReplyQu_[id].try_dequeue(entry)) {
      ...
      if (addr == NULL) {
        ...
        // Step 3: This replica has not received any entries from that proxy, so
        // it does not have any addr info Step 4: This replica wants to send
        // reply for this entry
      ...
      // If the owdMap_ does not have the proxyId (i.e. the owd for this proxyId
      // has not been estimated), it will return 0 (0 happens to be the dummy
      // value of protobuf, and the proxy will not consider it as an estimated
      // owd)
      reply.set_owd(owdMap_.get(entry->body.proxyId));
      SHA_HASH hash(entry->hash);
      hash.XOR(cv->cvHash_);
      if (amLeader) {
        // Leader's logic is very easy: after XORing the crashVector and the log
        // entry hash together, it can directly reply
        ...
        // remaining part (to the right of sync-point) to XOR the part that has
        // already been synced

        // Let's first get the boundary, i.e. minUnSyncedLogId_ and
        // maxSyncedLogId_ maxSynced is always updated earlier than minUnSynced,
        // so we first get minUnSynced, and then get maxSynced, this ensures
        // minUnSynced is no fresher than maxSynced By contrast, if we get the
        // two variables in the reverse order, then we cannot be sure which
        // variable is fresher, that can lead to the missing of some entries
        ...
        if (syncedLogId < CONCURRENT_MAP_START_INDEX ||
          ...
          // GarbageCollectionTd (we have advanced safeToClearUnSyncedLogId_) We
          // cannot decide the sync-point, so we directly reply with the XORed
          // hash (similar to the leader)
          reply.set_hash(hash.hash, SHA_DIGEST_LENGTH);
          ...
          if (entry->LessOrEqual(*syncedEntry)) {
            // No need to send fast replies, because this entry has already been
            // covered by index sync process, a slow reply has already been sent
            continue;
          }
          // Beyond syncedEntry, we need to find the boundary in the unsynced
          // logs We are sure that  unsyncedLogId >= CONCURRENT_MAP_START_INDEX
          ...
          // then unsyncedEntry may have already been surpasssed by syncedEntry,
          // we need to remove the (potential) overlap
...
void Replica::SlowReplyTd(int id) {
  ...
  while (status_ != ReplicaStatus::TERMINATED) {
    ...
    // TODO: SlowReplyTd can be merged with FastReplyTd, just needs to piggyback
    // the most recent synced-log-id on fast-reply, and that is enough for
    // proxies to do quorum check, even without slow-reply messages
...
void Replica::IndexSendTd(int id, int cvId) {
  ...
  uint64_t syncPeriod =
      replicaConfig_["sync-report-period-ms"].as<uint64_t>() * 1000;
  while (status_ != ReplicaStatus::TERMINATED) {
    BlockWhenStatusIsNot(ReplicaStatus::NORMAL);
    if (!AmLeader()) {
    ...
    if (missedReqMsg.ParseFromArray(msgBuffer, msgHdr->msgLen)) {
      for (int i = 0; i < missedReqMsg.reqs().size(); i++) {
        ...
        if (missedReqKeys_.find(rbMsg.reqkey()) != missedReqKeys_.end()) {
          ...
          // We must handle it to ProcessTd instead of processing it here, to
          // avoid data race (and further memroy leakage), although it is a
          // trivial possibility
          processQu_.enqueue(rb);
...
bool Replica::ProcessIndexSync(const IndexSync& idxSyncMsg) {
  ...
  for (uint32_t logId = maxSyncedLogId_ + 1; logId <= idxSyncMsg.logidend();
    ...
    if (reqLogId >= CONCURRENT_MAP_START_INDEX &&
      ...
      // unSyncedLogIdInUse_[fastReplyQu_.size()], otherwisxe, it may have been
      // reclaimed
    ...
    if (rb) {
      ...
      if (missedReqKeys_.empty()) {
        ...
        // Otherwise, only keep traversing to see how many requests are missing
        // and put the missing reqKeys in missedReqKeys_
...
void Replica::ReceiveAskMissedReq(MessageHeader* msgHdr, char* msgBuffer) {
  ...
  if (msgHdr->msgType == MessageType::MISSED_REQ_ASK &&
    ...
    if (missedReqMsg.reqs_size() > 0) {
      // This ack is useful because it really contains some missed requests, so
      // send it
      missedReqAckContext_->endPoint_->SendMsgTo(
          *(indexReceiver_[askReqMsg.replicaid()]), missedReqMsg,
          MessageType::MISSED_REQ);
...
void Replica::AskMissedIndex() {
  ...
  // Do not ask leader every time, choose random replica to ask to avoid leader
  // bottleneck
...
void Replica::PrepareNextReclaim() {
  ...
  while (lastPrepare <= maxUnSyncedLogId_ &&
    ...
    if ((!shouldReclaim) && entry->body.deadline + reclaimTimeout_ * 1000 <
      ...
      // follwoers for too long We assume reclaimTimeout_ is a sufficient time:
      // If leader has such an entry, it should have synced it with the follower
      // after such a long time Otherwise, it only proves that the leader does
      // not have such entry, so it is also unnecessary for the follower to keep
      // it
      shouldReclaim = true;
    }
    if (shouldReclaim) {
...
void Replica::CheckHeartBeat() {
  ...
  uint64_t nowTime = GetMicrosecondTimestamp();
  uint32_t threashold =
      replicaConfig_["heartbeat-threshold-ms"].as<uint32_t>() * 1000;
...
void Replica::InitiateViewChange(const uint32_t view) {
  ...
  viewId_ = view;
  // Unregister all timers, except the monitorTimer (so as the master thread can
  // break when status=Terminated)
  masterContext_->endPoint_->UnRegisterAllTimers();
  masterContext_->endPoint_->RegisterTimer(masterContext_->monitorTimer_);
...
void Replica::ProcessViewChangeReq(const ViewChangeRequest& viewChangeReq) {
  ...
  if (Aggregated(viewChangeReq.cv())) {
    // If cv is updated, then it is likely that some messages in viewChangeSet_
    // become stray, so remove them
    for (uint32_t i = 0; i < replicaNum_; i++) {
...
void Replica::TransferUnSyncedLog() {
  ...
  for (auto& kv : viewChangeSet_) {
    if (kv.second.lastnormalview() < largestNormalView) {
      // No need to transfer log, this guy's unsynced logs do not contribute to
      // committed logs
      continue;
    }
...
void Replica::SendStateTransferRequest() {
  ...
  for (auto& stateTransferInfo : stateTransferIndices_) {
    // Do not request too many entries at one time, otherwise, UDP packet cannot
    // handle that
...
void Replica::ProcessCommitInstruction(const CommitInstruction& commit) {
  ...
  if (commit.committedlogid() > committedLogId_) {
    // Don't assign committedLogId_ directly, because this replica may have not
    // get enough synced logs
...
bool Replica::Aggregated(const google::protobuf::RepeatedField<uint32_t>& cv) {
  ...
  if (needAggregate) {
    ...
    }  // Else (status_=ViewChange), then there is only master thread alive, no
       // need to wait for reply thread
  }
